...
% remains contant
%data from course video
R=110; % Range cannot exceed the max value of 200m
V=-20; %velocity can be any value in the range of -70 to + 70 m/s.
...
MixRs = reshape(Mix,[Nr, Nd]);
 % *%TODO* :
%run the FFT on the beat signal along the range bins dimension (Nr) and
%normalize.
Mix_fft = fft(MixRs,Nr)/Nr;
 % *%TODO* :
...
for irange = Tr + Gr + 1 : Nr/2 - Tr - Gr
    for idoppler = Td + Gd + 1 : Nd - Td - Gd
        ...
        training = db2pow(RDM(irange - Tr - Gr : irange + Tr + Gr, idoppler - Td - Gd : idoppler + Td + Gd));

        training(irange - Gr : irange + Gr,idoppler - Gd : idoppler + Gd) = 0;

        training = pow2db(sum(training) / ((2 * Tr + 2 * Gr + 1) * (2 * Td + 2 * Gd + 1) -((2 * Gr + 1) * (2 * Gd + 1))));
